...
const loginUser = async (req, res) => {
  ...
  if (!existUser)
    return res.status(404).json({ message: 'Usuario y/o contraseña no valida' })
  bcrypt
    ...
    registerUser
      ...
      .catch((error) => {
        return res.status(501).json({
          message: `Ha ocurrido un error al registar el usuario ${error}`
        })
      })
  } catch (error) {
    return res.status(501).json({
...
export { getUsers, getUser, loginUser, registerUser, updateUser, deleteUser }
